...
FlashIdea 的定位：**不做 AI 助手，只做语音便签本**。一个极简的聊天气泡界面，用户说话 → 语音转文字（由系统输入法完成，推荐豆包输入法）→ 自动 append 到飞书云文档。
...
这是 FlashIdea 最核心的 API 调用。每条消息发送时：
...
- **飞书录音豆集成**：通过飞书 CLI/API 拉取录音豆的转写文本，统一进入 FlashIdea pipeline
