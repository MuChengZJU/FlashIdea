...
- **同步失败显示错误原因**：消息同步失败时，红色文字直接显示具体原因（如"凭据无效"、"网络错误"、"知识库未配置"），不再只有一个感叹号
- **导出诊断日志**：设置页新增"导出诊断日志"按钮，一键复制到剪贴板，内测用户可粘贴发给开发者。包含版本号、配置状态、最近日志、失败消息摘要
- **启动日志记录**：SQLite `app_logs` 表持久化运行日志，方便事后排查

### 修复

- **"测试通过但发送失败"根因修复**：`test_connection` 不再写入全局 wiki 状态，改为用临时 client 测试。同时支持表单传参，未保存的配置也能直接测试
- **网络重试退避**：网络错误不再直接失败，改为指数退避重试（1s → 2s → 4s → 8s → 16s）
- **跨日文档 pull 修复**：`pull_remote_messages` 优先搜索当日飞书文档，不再依赖本地存储的过期 doc_id
- **config_from_env 判断逻辑**：改为 APP_ID 和 APP_SECRET 都存在才算环境变量配置，避免只设了一个就锁定 UI
- **secret_hint 短密钥保护**：≤4 位的 secret 只显示 `****`，不泄露原文
- **日志中消息内容脱敏**：导出日志中消息文本只显示前 2 字符 + `***`
- **wiki token 预览安全截取**：用 `.chars().take(8)` 替代字节切片，避免中文字符截断 panic

### Codex 审阅流程

本版本首次使用 Codex review → 写修复方案 → Codex 审阅方案 → 并行 TDD worktree 的流程：
